...
Public Sub ZoteroLinkCitationAll()
    ...
    ' 全文运行：不 debug，notify=True 只会：
    ' 1）开始时问一次是否给超链接应用某个样式
    ' 2）结束时弹一次“Linked X Zotero citations.”
    Call ZoteroLinkCitation(ActiveDocument.Fields, False, True)
    ...
    For Each aField In targetFields
        ...
        If aField.Type = wdFieldAddin Then
            If InStr(aField.code, "ADDIN ZOTERO_ITEM") > 0 Then
                ...
                If debugging Then
                    ' 只有 debugging=True 时才会逐个弹窗，这里我们在调用时已经固定为 False
                    Application.ScreenUpdating = True
